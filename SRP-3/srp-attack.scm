(herald
 "SRP-3 protocol with malicious server "
 (algebra diffie-hellman)
 (bound 40)
 (limit 8000)
 )

(defprotocol srp3 diffie-hellman
  ...
  ; This role is a server with only server knowledge acting as though it is a client
  ; If there are shapes that complete with a malicious server and a regular server, then
  ; a server can be used to authenticate as a client to itself (possibly via a malicious process)
  (defrole malserver
    (vars (client server name) (a rndx) (b u expt) (s text) (v base))
    (trace
     (send client) ; MalServer sends client's name
     (recv s) ; MalServer receives Client's salt
     (obsv (cat "Server record" s v client server)) ; MalServer retreives Client's verifier and salt
     (send (exp (gen) a)) ; MalServer sends A = g^a, a is generated freshly by MalServer
     (recv (cat (enc (exp (gen) b) v) u)) ; MalServer receives B = g^b encrypted by v, u
     ; MalServer sends H(A, B, K) = M1
     (send (hash (exp (gen) a) ; A
		 (enc (exp (gen) b) v) u ; B
		 (hash (exp (gen) (mul a b)) (exp v (mul u b))))) ; K = (A^b)((v^u)^b)
     ; MalServer receives H(A, M1, K) = M2
     (recv (hash (exp (gen) a) ; A
		 (hash (exp (gen) a) ; M1 begins
		       (enc (exp (gen) b) v) u ; B
  ...
  (comment "This protocol is a rewritten version of the SRP-3 model originally produced by Moses Liskov")
...
(defskeleton srp3 ; Analysis of malserver run
...
; Uncomment below to find all shapes for protocol including non-attack shapes
;(defskeleton srp3 ; Analysis of server run
;  (vars (client server name))
;  (defstrand server 7 (server server) (client client))
;  (non-orig (ltk client server)))

;(defskeleton srp3 ; Analysis of client run
;  (vars (client server name))
;  (defstrand server 7 (server server) (client client))
;  (non-orig (ltk client server)))
