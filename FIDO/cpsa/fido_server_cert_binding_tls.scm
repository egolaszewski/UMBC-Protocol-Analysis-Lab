(herald "FIDO with TLS authentication of server and server certificate binding."
	(bound 20))
...
;;; Bind the challenge to the TLS server certificate.
...
(defprotocol fido basic
  ...
  (defrole server
    ...
    (trace
     ...
     (recv (enc (signed (channel_binding_servercert challenge server pks ca) auth)
  ...
  (defrole client
    ...
    (trace
     ...
     (send (enc (channel_binding_servercert challenge server pks ca) (ltk client auth)))
     (recv (enc (signed (channel_binding_servercert challenge server pks ca) auth) (ltk client auth)))
     (send (enc (signed (channel_binding_servercert challenge server pks ca) auth)
		(ClientWriteKey (MasterSecret pms n1 n2))))
     )
    )
