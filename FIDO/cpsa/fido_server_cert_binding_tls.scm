...
(defmacro (channel_binding_servercert challenge server ca)
  (hash challenge (Certificate server ca))
)
...
(defprotocol fido basic
  ...
  (defrole server
    ...
    (trace
     ...
     (recv (enc (signed (channel_binding_servercert challenge server ca) auth)
  ...
  (defrole client
    ...
    (trace
     ...
     (send (enc (channel_binding_servercert challenge server ca) (ltk client auth)))
     (recv (enc (signed (channel_binding_servercert challenge server ca) auth) (ltk client auth)))
     (send (enc (signed (channel_binding_servercert challenge server ca) auth)
		(ClientWriteKey (MasterSecret pms n1 n2))))
     )
    (non-orig (privk ca))
    )
