...
(defprotocol uaf-unbound-tls13 basic
  ...
  (defrole client-reg
    ...
    (trace
      ...
      (UAF_Reg_Unbound_TLS13 send recv username appid challenge auth authk cr sr spk y x)
    )
    (uniq-gen x)
  )
  (defrole client-auth
    ...
    (trace
      (TLS send recv cr sr x y server spk (invk spk) ca (HandshakeSecret (DHpublic y) x))
      (UAF_Auth_Unbound_TLS13 send recv appid challenge authk cr sr spk y x)
    )
    (uniq-gen x)
  )
  ;; Server roles.
  (defrole server-reg
    ...
    (trace
      ...
      (UAF_Reg_Unbound_TLS13 recv send username appid challenge auth authk cr sr spk x y)
    )
    (uniq-gen y)
  )
  (defrole server-auth
    ...
    (trace
      (TLS recv send cr sr x y server spk (invk spk) ca (HandshakeSecret (DHpublic x) y))
      (UAF_Auth_Unbound_TLS13 recv send appid challenge authk cr sr spk x y)
    )
    (uniq-gen y)
  )
  ;; Custom sorts.
...
(defgoal uaf-unbound-tls13
  (forall
    ((sr random32) (challenge text) (authk spk akey) (server ca appid name)
      (y rndx) (z strd))
    (implies
      (and
        (p "server-reg" z 5)
