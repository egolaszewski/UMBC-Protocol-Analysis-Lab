...
(defprotocol uaf-unbound-tls12 basic
  ...
  (defrole client-reg
    ...
    (trace
      ...
      (UAF_Reg_EndPoint_TLS12 send recv server ca username appid challenge auth authk cr sr pms)
    )
  )
  (defrole client-auth
    (vars
    ...
    (trace
      (TLS send recv pms cr sr server ca)
      (UAF_Auth_EndPoint_TLS12 send recv server ca appid challenge authk cr sr pms)
    )
  )
  ;; Server roles.
  (defrole server-reg
    ...
    (trace
      ...
      (UAF_Reg_EndPoint_TLS12 recv send server ca username appid challenge auth authk cr sr pms)
    )
  )
  (defrole server-auth
    (vars
    ...
    (trace
      (TLS recv send pms cr sr server ca)
      (UAF_Auth_EndPoint_TLS12 recv send server ca appid challenge authk cr sr pms)
    )
  )
  ;; Custom sorts.
  (lang
