...
(defprotocol sbp basic
  ...
  (defrole proxy
    ...
    (trace
     ...
     (TLS send recv ppms pr2 sr s ca)
     (send (enc "login" c p (cwk ppms pr2 sr)))
     ...
     (send (enc request cookie (cwk ppms pr2 sr)))
     (recv (enc response (cwk ppms pr2 sr)))
  ...
  (defrole server
    ...
    (trace
     ...
     (recv (enc request cookie (cwk pms cr sr)))
...
(defskeleton sbp
  ...
  (defstrand server 8 (s s) (ca ca) (sr sr) (p p) (cookie cookie))
  (non-orig (privk s) (privk ca))
  (uniq-orig sr)
  (pen-non-orig p)
  )
