(herald "Session Binding Protocol with TLS 1.2 (proxy and server seperate as in paper description (mTLS connection from proxy to server) with server on private network.)"
	(bound 12)
	(limit 4000)
	)
...
(defprotocol sbp basic
  ...
  (defrole proxy
    ...
    (conf ln)
    (uniq-orig ppms pr1 pr2) ; assumptions of a trusted proxy
...
; Cannot assume that the server generates a fresh nonce since the client only sees
...
(defskeleton sbp
  (vars (prxy ca name) (cr pr1 random32) (pms random48) (p password))
  (defstrandmax client (s prxy) (ca ca) (cr cr) (sr pr1) (pms pms) (p p))
  (non-orig (privk prxy) (privk ca))
  (uniq-orig cr pr1 pms)
  (pen-non-orig p)
  )
